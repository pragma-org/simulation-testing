...
In psuedo code we can define our echo node like this:
...
Given how trivial the example is, it might almost seem a bit unneccessary, but
since the testing setup is the same as for more complicated examples it's still
...
with the psuedo code we sketched before.

One thing to note is that th structure of the main function will be the same
for other examples, so Maelstrom provides a node absraction which let's us
merely write the block that below the comment "essense of echo". Here's the
...
Basically the node abstraction does what `stdio` did in our psuedo code, i.e.
...
With the dependencies installled, we can test the echo example as follows.
...
I've edited the output to keep it breif. From the `:stats` we can see that
...
deterministic way. We'll then reimplement the message generation part of
workloads to be completely deterministic as well. Since execution of the tests
will be deterministic, we can also implement shrinling and present small
counterexamples.
