...
the [next post](02-maelstrom-testing-echo-example.md) where we'll start by
Jepsen testing a simple echo node example as a warm up for then later
simulation testing the exact same example.
...
[^6]: In another [discussion](https://youtu.be/JoYjji1DZCE?t=2346) Joran says
    that their simulator runs at 700x real-time and then parallelised over 1000
    cores, so for each second in real time their simulation simulates 8.1 days
